...
public class ItineraryDay4 extends Fragment {
    private View view;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_itinerary_day4, container, false);
        //CHECK NET TOAST @PRAKASH
        if (!isNetworkOnline()==true)
        {
            Toast.makeText(getActivity(), "No Network Connection For Feeds",
                    Toast.LENGTH_LONG).show();
            ProgressBar pb = (ProgressBar) view.findViewById(R.id.day4_progressBar);
            pb.setVisibility(View.INVISIBLE);
        }
        else {
            new Day4Async().execute("http://iidea8.webuda.com/services/itenary_service.php?date=2015-10-26");
        }
        setRetainInstance(true);
        return view;
    }

    public class Day4Async extends AsyncTask<String, Void, ArrayList<DaysGnS>> {
    ...
    public void onResume() {
        super.onResume();
        MyApplication.getInstance().trackScreenView("Itinerary Day 4");
    }
